    ...
    try:
        ...
        try:
            ...
            if len(X_scaled) > 5000:
                print(
                    "   (Large dataset detected, using sample for faster computation...)"
                )
                ...
                print(
                    f"   ✅ Silhouette Score (on {sample_size} sample): {silhouette_avg:.3f}"
                )
            else:
            ...
            for cluster_id, count in cluster_counts.items():
                print(
                    f"  Cluster {cluster_id}: {count} songs ({count/len(df)*100:.1f}%)"
                )
            ...
            for cluster_id in range(n_clusters):
                ...
                if len(cluster_data) > 0:
                    ...
                    for genre, count in genre_dist.items():
                        print(
                            f"    {genre}: {count} ({count/len(cluster_data)*100:.1f}%)"
                        )
                else:
