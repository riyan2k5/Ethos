...
def find_similar_songs(track_id: str, n_similar: int = 10):
    ...
    track_features = track_features_df[feature_columns].fillna(
        track_features_df[feature_columns].median()
    )
    track_features_scaled = scaler.transform(track_features)

    # Find similar songs using KNN
    distances, indices = knn_model.kneighbors(
        track_features_scaled, n_neighbors=n_similar + 1  # +1 to exclude itself
    )
    ...
    for i, row in results_df.iterrows():
        ...
        print(
            f"   Genre: {row['genre']}, Similarity: {row['similarity_score']:.3f}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--track-id", type=str, required=True, help="Track ID to find similar songs for"
    )
    parser.add_argument(
        "--n-similar", type=int, default=10, help="Number of similar songs to return"
    )
