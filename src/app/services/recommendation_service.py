...
class RecommendationService:
    ...
    def __init__(self, data_loader: DataLoader, ml_service: MLService, spotify_service: SpotifyService):
    ...
    def get_similar_songs(self, track_id: str, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            if len(similar) < limit:
                similar_from_dataset = self._find_similar_in_dataset(song, df, limit, track_id)
            ...
            results = []
            seen_track_ids = {track_id}  # Track IDs we've already added (including the current song)
            ...
            for sim_song in similar:
                ...
                if full_song:
                    ...
                    results.append({
                        ...
                        'track_name': full_song.get('track_name', full_song.get('title', sim_song.get('track_name', 'Unknown'))),
                        'artist_name': full_song.get('artist_name', full_song.get('artists', sim_song.get('artist_name', 'Unknown'))),
    ...
    def _find_similar_in_dataset(self, song: Dict, df, limit: int, exclude_track_id: str) -> List[Dict]:
