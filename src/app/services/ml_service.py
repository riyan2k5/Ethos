...
class MLService:
    ...
    def load_models(self):
        ...
        if all(
            f.exists()
            for f in [similar_model_file, similar_scaler_file, similar_features_file]
        ):
    ...
    def _create_engineered_features(self, features: Dict) -> Dict:
        ...
        if "danceability" in features and "energy" in features:
            engineered["Dance_Energy_Interaction"] = (
                features["danceability"] * features["energy"]
            )

        if "valence" in features and "energy" in features:
            engineered["Valence_Energy_Ratio"] = features["valence"] / (
                features["energy"] + 1e-6
            )

        if "acousticness" in features and "instrumentalness" in features:
            engineered["Acoustic_Instrumental_Interaction"] = (
                features["acousticness"] * features["instrumentalness"]
            )

        if "speechiness" in features and "energy" in features:
            engineered["Speechiness_Energy_Ratio"] = features["speechiness"] / (
                features["energy"] + 1e-6
            )
    ...
    def find_similar_songs(
        self, song: Dict, limit: int = 10, available_track_ids: set = None
    ) -> List[Dict]:
        ...
        try:
            ...
            feature_vector = [
                features.get(feat, 0.0) for feat in self.similar_songs_features
            ]
            # Create DataFrame with proper column names
            feature_df = pd.DataFrame(
                [feature_vector], columns=self.similar_songs_features
            )
