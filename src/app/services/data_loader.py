...
class DataLoader:
    ...
    def __init__(self):
        ...
        self.dataset_path = Path(__file__).parent.parent.parent.parent / "data" / "fortest" / "Spotify_Dataset_V3.csv"
    ...
    def load_custom_dataset(self, file_content: bytes, filename: str) -> Dict[str, any]:
        ...
        try:
            ...
            for delimiter in [';', ',', '\t']:
                try:
                    df = pd.read_csv(io.BytesIO(file_content), delimiter=delimiter, encoding=encoding, low_memory=False)
            ...
            if df is None:
                raise ValueError("Could not parse CSV file. Please ensure it's a valid CSV.")
            
            # Normalize column names (same as default dataset)
            df.columns = [col.lower().replace(' ', '_').replace('.', '').replace('#_of', 'num_of').replace('(ind)', 'ind') for col in df.columns]
            ...
            if 'track_id' not in df.columns:
                ...
                possible_ids = [col for col in df.columns if 'id' in col.lower() or 'track' in col.lower()]
                if possible_ids:
            ...
            # Fill missing values
            numerical_cols = ['danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'valence']
