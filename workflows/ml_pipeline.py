...
def train_genre_model_task(
    table_name: str, target_col: str = "genre"
) -> Dict[str, Any]:
    """
...
def train_energy_regression_model_task(
    table_name: str, target_col: str = "energy"
) -> Dict[str, Any]:
    """
...
def train_popularity_regression_model_task(
    table_name: str, target_col: str = "popularity"
) -> Dict[str, Any]:
    """
...
def ml_training_pipeline(
    ...
    try:
        ...
        try:
            popularity_result = train_popularity_regression_model_task(
        ...
        successful_models = [
            name
            for name, result in results["model_training"].items()
        ...
        logger.info(
            f"✅ Successful models: {len(successful_models)}/{len(results['model_training'])}"
        )
        ...
        if failed_models:
            logger.warning(
                f"❌ Failed models: {len(failed_models)}/{len(results['model_training'])}"
            )
            ...
            try:
                if failed_models:
                    ...
                    notification_sent = notification_service.notify_success(
                        "ML Training Pipeline", duration, details
                    )
                    results["notification_sent"] = notification_sent
            except Exception as e:
        ...
        if notification_service:
            try:
                ...
                notification_sent = notification_service.notify_failure(
                    "ML Training Pipeline", error_msg, details
                )
                results["notification_sent"] = notification_sent
...
if __name__ == "__main__":
    # Run the pipeline
    result = ml_training_pipeline(table_name=TABLE_NAME, n_clusters=N_CLUSTERS)
    print("\nPipeline Results:")
    print(result)
