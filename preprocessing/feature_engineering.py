...
def create_new_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Valence_Energy_Ratio'] = df['valence'] / (df['energy'] + 1e-6)  # Add small epsilon to prevent division by zero
    
    # Handle any infinite or NaN values
    df = df.replace([np.inf, -np.inf], np.nan)
    
    # Fill NaN values in new numerical columns with median
    new_cols = ['Dance_Energy_Interaction', 'Loudness_Duration_Ratio', 
                'Tempo_vs_120', 'Valence_Energy_Ratio']
    for col in new_cols:
